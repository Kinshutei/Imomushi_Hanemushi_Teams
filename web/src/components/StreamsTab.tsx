  ...
  const filteredStreams = isSearching
    ? streams.filter((stream) =>
        records
          ...
          .some((r) => r.楽曲名.toLowerCase().includes(trimmedQuery.toLowerCase()) || r.原曲アーティスト.toLowerCase().includes(trimmedQuery.toLowerCase()))
      )
    : streams
        ...
        <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', width: '100%', maxWidth: '360px' }}>
          ...
          <input
            ...
            style={{
              ...
              background: '#1c1c1c',
              color: '#e8e8e8',
              boxShadow: isSearching ? '0 0 0 2px rgba(58,143,85,0.25)' : undefined,
              borderColor: isSearching ? '#3a8f55' : '#2e2e2e',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
        {filteredStreams.map((stream) => {
          const setlist = records
            .filter((r) => r.枠名 === stream.枠名)
            .filter((r) => !isSearching || r.楽曲名.toLowerCase().includes(trimmedQuery.toLowerCase()) || r.原曲アーティスト.toLowerCase().includes(trimmedQuery.toLowerCase()))
            ...
            <div>
              {thumbUrl ? (
                <>
                  ...
                  <a href={cleanUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: '12px', color: '#3a8f55', display: 'block', marginTop: '4px' }}>
                    ▶ YouTubeで開く
                  </a>
                </>
              ) : (
